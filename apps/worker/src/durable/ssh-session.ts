...
export class SSHSessionDO implements DurableObject {
  ...
  constructor(
    ...
    this.dependencies = {
      createProfileRepository: dependencies.createProfileRepository ?? ((runtimeEnv) => new D1SSHProfileRepository(runtimeEnv)),
      createHostKeyRepository: dependencies.createHostKeyRepository ?? ((runtimeEnv) => new D1HostKeyRepository(runtimeEnv.DB)),
      createEngine: dependencies.createEngine ?? ((engineDependencies) => new SSH2Engine(engineDependencies))
    };
    ...
    void state.blockConcurrencyWhile(async () => {
      await Promise.all(state.getWebSockets().map((socket) => this.recoverRestartedSocket(socket)));
    });
  }

  async fetch(request: Request): Promise<Response> {
    const url = new URL(request.url);
    if (url.pathname === "/ticket" && request.method === "POST") return this.issueTicket(request);
    if (url.pathname === "/connect" && request.method === "GET") return this.upgrade(request, url);
  ...
  async webSocketMessage(socket: WebSocket, raw: string | ArrayBuffer): Promise<void> {
    try {
      ...
      if (new TextEncoder().encode(raw).byteLength > MAX_JSON_MESSAGE_BYTES) {
        throw new ProtocolError("BAD_REQUEST", "WebSocket message exceeds 64 KiB", true, WEBSOCKET_CLOSE_CODES.MESSAGE_TOO_LARGE);
      }
      let decoded: unknown;
  ...
  async webSocketClose(socket: WebSocket, code: number, _reason: string, wasClean: boolean): Promise<void> {
    ...
    await this.cleanup(socket, {
      ...
      message: userInitiated ? "Browser WebSocket closed" : "Browser WebSocket closed unexpectedly"
    });
  }
  ...
  private async issueTicket(request: Request): Promise<Response> {
    ...
    const origin = parseVerifiedOrigin(request.headers.get("x-verified-origin"));
    if (!origin) return Response.json({ error: "A verified origin is required" }, { status: 400 });
    ...
    try {
      ...
      const session = raw.sessionId === undefined ? { success: true as const, data: crypto.randomUUID() } : EntityIdSchema.safeParse(raw.sessionId);
      ...
      if (!owner.success || !session.success || !ticket.success) throw new Error("Invalid internal ticket request");
      body = { ownerId: owner.data, sessionId: session.data, request: ticket.data };
    } catch {
      return Response.json({ error: "Invalid internal ticket request" }, { status: 400 });
    }

    let profile: SSHConnectionProfile;
    try {
      profile = await this.dependencies.createProfileRepository(this.env).resolve(
        body.ownerId,
    ...
    const aad = ticketAdditionalData(sessionId, body.request.attemptId, expiresAt, origin);
    try {
      ...
      if (!created) return Response.json({ error: "A ticket has already been issued for this session" }, { status: 409 });
      return Response.json({ ticket, expiresAt: new Date(expiresAt).toISOString(), sessionId });
    } finally {
      ticketBytes.fill(0);
    }
  }
  ...
  private async upgrade(request: Request, url: URL): Promise<Response> {
    ...
    const ticket = url.searchParams.get("ticket") ?? request.headers.get("x-session-ticket");
    if (!ticket || !origin || !ownerId) return Response.json({ error: "Invalid SSH ticket" }, { status: 401 });
    const authorization = await this.consumeTicket(ticket, origin, ownerId);
    if (!authorization) return Response.json({ error: "Invalid, expired, or already used SSH ticket" }, { status: 401 });
    ...
    this.deadlines.set(server, setTimeout(() => {
      void this.closeSocket(server, WEBSOCKET_CLOSE_CODES.MESSAGE_INVALID, "Connect message timed out");
    }, CONNECT_MESSAGE_TIMEOUT_MS));
    return new Response(null, { status: 101, webSocket: client });
  }

  private async consumeTicket(ticket: string, origin: string, ownerId: string): Promise<TicketAuthorization | null> {
    ...
    try {
      ...
      if (!stored || stored.expiresAt < Date.now() || stored.expiresAt > Date.now() + TICKET_TTL_MS + 5_000
        || stored.origin !== origin || !constantTimeTextEqual(stored.ownerId, ownerId)
        || !constantTimeTextEqual(stored.ticketHash, presentedHash)) return null;
      const profile = await openProfile(
        ticketBytes,
        stored.profile,
        ticketAdditionalData(stored.sessionId, stored.attemptId, stored.expiresAt, stored.origin)
      );
      return {
  ...
  private async handleClientMessage(socket: WebSocket, message: ClientWebSocketMessage): Promise<void> {
    const authorization = this.authorizations.get(socket);
    const live = this.sessions.get(socket);
    if (!authorization && !live) throw new ProtocolError("SSH_TICKET_INVALID", "SSH session authorization is missing");
    const context = live?.authorization ?? authorization;
    if (!context || ("attemptId" in message && message.attemptId !== context.attemptId)) {
      throw new ProtocolError("VALIDATION_FAILED", "WebSocket attempt ID does not match the ticket");
    }

    if (message.type === "hello") {
      if (live) throw new ProtocolError("VALIDATION_FAILED", "SSH session has already started");
      this.sendStatus(socket, context, "authorizing", "SSH ticket accepted", message.requestId);
      return;
    }

    if (message.type === "connect") {
      if (live || !authorization || this.startingSockets.has(socket)) {
        throw new ProtocolError("VALIDATION_FAILED", "SSH session has already started");
      }
      ...
      try {
        ...
        if (!leaseAcquired) {
          ...
          this.sendError(socket, authorization, "RATE_LIMITED", "The active SSH session limit has been reached", true);
          await this.closeSocket(socket, WEBSOCKET_CLOSE_CODES.SESSION_TERMINATED, "SSH session limit reached");
          return;
        }
        if (socket.readyState !== WebSocket.OPEN || this.authorizations.get(socket) !== authorization) {
          await this.releaseSessionLease(authorization.profile.ownerId, authorization.sessionId);
          return;
        }

        audit = await SSHSessionAudit.start(this.env, authorization.sessionId, authorization.profile);
        if (socket.readyState !== WebSocket.OPEN || this.authorizations.get(socket) !== authorization) {
          await Promise.allSettled([
            this.releaseSessionLease(authorization.profile.ownerId, authorization.sessionId),
            audit.finish("closed", "user_disconnect", "Browser WebSocket closed during SSH startup")
          ]);
          return;
        }

        const engine = this.dependencies.createEngine({
          transportFactory: createSSHTransportFactory(config, authorization.sessionId),
        ...
        void engine.connect(authorization.profile, {
          ...
          session.metricsTimer = setInterval(() => void this.collectMetrics(socket, session), METRICS_INTERVAL_MS);
          void this.collectMetrics(socket, session);
        }).catch(() => undefined);
      } catch (error) {
        if (audit) {
          await audit.finish("error", "internal_error", "SSH session initialization failed");
        }
        if (leaseAcquired) {
          await this.releaseSessionLease(authorization.profile.ownerId, authorization.sessionId);
        }
    ...
    if (!live) throw new ProtocolError("VALIDATION_FAILED", "The first session command must be connect");
    switch (message.type) {
      case "host-key-decision":
        ...
        if (message.decision === "reject") {
          live.audit.state("error", "host_key_rejected", "SSH host key was rejected by the user");
          live.audit.security("ssh_host_key_rejected", `SSH host key rejected for profile ${context.profile.profileName}`);
        } else {
          live.audit.state("ssh_handshake", "host_key_accepted", "SSH host key was accepted by the user");
        }
        break;
      case "input": {
      ...
      case "sftp-list": {
        ...
        if (message.cursor !== undefined && !/^\d{1,10}$/.test(message.cursor)) {
          throw new ProtocolError("VALIDATION_FAILED", "Invalid directory cursor", false);
        }
        const pageStart = message.cursor === undefined ? 0 : Number(message.cursor);
        if (!Number.isSafeInteger(pageStart) || pageStart < 0 || pageStart > entries.length) {
          throw new ProtocolError("VALIDATION_FAILED", "Invalid directory cursor", false);
        }
        ...
        this.sendServer(socket, {
          ...
          operation: "list",
          path: message.path,
          entries: pageEntries.map(({ type, ...entry }) => ({ ...entry, kind: type })),
      ...
      case "sftp-read": {
        ...
        if (file.size > message.maxBytes) {
          throw new ProtocolError("SFTP_FILE_TOO_LARGE", `Remote file exceeds the ${message.maxBytes} byte text limit`, false);
        }
        this.sendServer(socket, {
          protocolVersion: WS_PROTOCOL_VERSION,
          type: "file-result",
      ...
      case "sftp-write":
        await this.assertWriteVersion(live, message.path, message.expectedSize, message.expectedModifiedAt);
        await this.startUpload(socket, live, message.requestId, message.path, message.size, "write");
        break;
      case "sftp-upload-start":
        await this.startUpload(socket, live, message.requestId, message.path, message.size, "upload");
        break;
      ...
      case "sftp-mkdir":
        ...
        this.sendFileMutation(socket, live, message.requestId, "mkdir", message.path);
        break;
      case "sftp-rename":
        await live.engine.rename(message.sourcePath, message.destinationPath);
        this.sendFileMutation(socket, live, message.requestId, "rename", message.sourcePath, message.destinationPath);
        break;
      case "sftp-delete":
        await live.engine.deletePath(message.path, message.kind);
        this.sendFileMutation(socket, live, message.requestId, "delete", message.path);
        break;
      case "sftp-chmod":
        await live.engine.chmod(message.path, message.mode);
        this.sendFileMutation(socket, live, message.requestId, "chmod", message.path);
        break;
      default:
        throw new ProtocolError("SFTP_NOT_AVAILABLE", `${message.type} is not implemented by the basic SFTP engine`, false);
    }
  }

  private async handleBinaryFrame(socket: WebSocket, bytes: Uint8Array): Promise<void> {
    const live = this.sessions.get(socket);
    if (!live) throw new ProtocolError("VALIDATION_FAILED", "SSH session is not ready");
    const { header, payload } = decodeBinaryFrame(bytes);
    if (header.sessionId !== live.authorization.sessionId) throw new ProtocolError("VALIDATION_FAILED", "Binary frame session ID mismatch");
    if (header.kind === "terminal-input") {
      live.commandCapture.feed(payload);
      await live.engine.input(payload);
      return;
    }
    if (header.kind !== "sftp-upload-chunk" || !header.transferId || header.offset === undefined) {
      throw new ProtocolError("VALIDATION_FAILED", "Unsupported client binary frame");
    }
    const transfer = live.uploads.get(header.transferId);
    if (!transfer || transfer.offset !== header.offset) throw new ProtocolError("VALIDATION_FAILED", "Unexpected upload offset", false);
    if (transfer.offset + payload.byteLength > transfer.size) throw new ProtocolError("VALIDATION_FAILED", "Upload exceeds declared size", false);
    const done = transfer.offset + payload.byteLength === transfer.size;
    transfer.offset = await live.engine.upload(header.transferId, transfer.path, transfer.offset, payload, done);
    if (done) {
    ...
    this.sendTransferProgress(socket, live, header.transferId, "upload", done ? "completed" : "transferring", transfer);
  }
  ...
  private async startUpload(
    ...
    const transfer: UploadTransfer = { requestId, path, size, offset: 0, startedAt: Date.now(), kind };
    ...
    if (size === 0) {
      ...
      this.sendTransferProgress(socket, live, transferId, "upload", "completed", transfer);
      return;
    }
    this.sendTransferProgress(socket, live, transferId, "upload", "transferring", transfer);
  }

  private async startDownload(socket: WebSocket, live: LiveSession, requestId: string, path: string): Promise<void> {
    const transferId = crypto.randomUUID();
    ...
    const finalOffset = await live.engine.download(path, {
      ...
      onChunk: async (chunk, offset) => {
        for (let cursor = 0; cursor < chunk.byteLength; cursor += MAX_TERMINAL_CHUNK_BYTES) {
          const part = chunk.subarray(cursor, Math.min(chunk.byteLength, cursor + MAX_TERMINAL_CHUNK_BYTES));
    ...
    const transfer: UploadTransfer = { requestId, path, size: totalBytes, offset: finalOffset, startedAt, kind: "upload" };
    this.sendTransferProgress(socket, live, transferId, "download", "completed", transfer);
  }
  ...
  private handleEngineEvent(socket: WebSocket, event: SSHEngineEvent): void {
    ...
    if (event.type === "output") {
      for (const command of live.commandCapture.observeOutput(event.data)) live.audit.command(command);
      for (let offset = 0; offset < event.data.length; offset += MAX_JSON_OUTPUT_CHARS) {
    ...
    if (event.type === "banner") {
      if (event.message) this.handleEngineEvent(socket, { type: "output", data: `${event.message}\r\n` });
      return;
    }
    if (event.type === "status") {
      this.sendStatus(socket, context, mapEngineState(event.phase), event.message, live.requestId);
      if (event.phase === "tcp_connecting") {
        live.audit.state("tcp_connecting", "tcp_connecting", "Opening the SSH TCP connection");
      } else if (event.phase === "ssh_handshake") {
        live.audit.state("ssh_handshake", "ssh_handshake_started", "SSH transport handshake started");
      } else if (event.phase.includes("auth")) {
        live.audit.state("authenticating", "authentication_started", "SSH authentication started");
    ...
    if (event.type === "host_key") {
      ...
      if (event.trusted) {
        live.audit.state("ssh_handshake", "host_key_accepted", "Stored SSH host key matched");
        return;
      }
      if (event.previousFingerprint) {
        live.audit.security("ssh_host_key_changed", `SSH host key changed for profile ${context.profile.profileName}`);
        return;
      }
      live.audit.state("host_confirmation", "host_key_confirmation_required", "SSH host key confirmation is required");
      this.sendStatus(socket, context, "host_confirmation", "SSH host key confirmation is required", live.requestId);
      this.sendServer(socket, {
        protocolVersion: WS_PROTOCOL_VERSION,
        type: "host-key",
        sessionId: context.sessionId,
        ...
        confirmationExpiresAt: new Date(Date.now() + HOST_CONFIRMATION_TIMEOUT_MS).toISOString()
      });
      return;
    }
    if (event.type === "error") {
      const closure = classifyEngineFailure(event.code, event.message);
      const eventCode = closure.closeReason === "authentication_failed" ? "authentication_failed"
        : closure.closeReason === "host_key_rejected" ? "host_key_rejected"
          : closure.closeReason === "keepalive_failed" ? "keepalive_failed" : "error";
      live.audit.state("error", eventCode, closure.message);
      if (closure.closeReason === "host_key_rejected") {
        live.audit.security("ssh_host_key_rejected", `SSH host key validation failed for profile ${context.profile.profileName}`);
      }
      const code = event.code.includes("HOST_KEY") ? (event.code.includes("MISMATCH") ? "SSH_HOST_KEY_CHANGED" : "SSH_HOST_KEY_REJECTED") : "SSH_CONNECTION_FAILED";
      this.sendError(socket, context, code, event.message, true);
      void this.closeSocket(socket, WEBSOCKET_CLOSE_CODES.SESSION_TERMINATED, event.message, closure);
      return;
    }
    if (event.type === "closed") {
      this.sendStatus(socket, context, "closed", event.reason, live.requestId, "remote_closed");
  ...
  private async renewSessionLease(socket: WebSocket, live: LiveSession): Promise<void> {
    ...
    try {
      ...
      if (!renewed.acquired && this.sessions.get(socket) === live) {
        this.sendError(socket, live.authorization, "SERVICE_UNAVAILABLE", "SSH session lease expired", true);
        await this.closeSocket(socket, WEBSOCKET_CLOSE_CODES.SESSION_TERMINATED, "SSH session lease expired", {
          finalState: "error",
      ...
      if (this.sessions.get(socket) === live) {
        ...
        if (remainingMs > 5_000) {
          this.scheduleLeaseRenewal(socket, live, Math.min(5_000, remainingMs - 5_000));
        } else {
          this.sendError(socket, live.authorization, "SERVICE_UNAVAILABLE", "Unable to renew the SSH session lease", true);
          await this.closeSocket(socket, WEBSOCKET_CLOSE_CODES.SESSION_TERMINATED, "SSH session lease renewal failed", {
            finalState: "error",
            closeReason: "internal_error",
  ...
  private scheduleLeaseRenewal(socket: WebSocket, live: LiveSession, delayMs: number): void {
  ...
  private async collectMetrics(socket: WebSocket, live: LiveSession): Promise<void> {
    if (this.sessions.get(socket) !== live || live.engine.state !== "ready") return;
    try {
      const snapshot = await live.engine.execMetrics();
      if (this.sessions.get(socket) === live) this.sendMetrics(socket, live.authorization, snapshot);
    } catch {
      // Monitoring is optional and must not terminate a healthy terminal.
    }
  }

  private async assertWriteVersion(live: LiveSession, path: string, expectedSize: number, expectedModifiedAt: string): Promise<void> {
    const metadata = await live.engine.stat(path);
    if (metadata.size !== expectedSize || metadata.modifiedAt !== expectedModifiedAt) {
      throw new ProtocolError("SFTP_CONFLICT", "Remote file changed after it was opened", false);
    }
  }
  ...
  private sendStatus(
    ...
    sessionState: "authorizing" | "tcp_connecting" | "ssh_handshake" | "host_confirmation" | "authenticating" | "connected" | "closed" | "error",
    message: string,
    requestId?: string,
  ...
  private sendTransferProgress(
    ...
    const elapsedSeconds = Math.max(0.001, (Date.now() - transfer.startedAt) / 1_000);
    this.sendServer(socket, {
      protocolVersion: WS_PROTOCOL_VERSION,
      ...
      estimatedSecondsRemaining: transfer.offset > 0 ? Math.max(0, (transfer.size - transfer.offset) / (transfer.offset / elapsedSeconds)) : null,
  ...
  private sendServer(socket: WebSocket, message: ServerWebSocketMessage | Record<string, unknown>): void {
    if (socket.readyState !== WebSocket.OPEN) return;
    const parsed = ServerWebSocketMessageSchema.safeParse(message);
    if (!parsed.success) throw new Error("Attempted to send an invalid server WebSocket message");
  ...
  private async waitForBackpressure(socket: WebSocket): Promise<void> {
    ...
    while (socket.bufferedAmount > MAX_BUFFERED_BYTES) {
      if (socket.readyState !== WebSocket.OPEN || Date.now() >= deadline) throw new Error("WebSocket backpressure limit exceeded");
      await new Promise((resolve) => setTimeout(resolve, 10));
    }
  }

  private async closeSocket(socket: WebSocket, code: number, reason: string, closure: SessionClosure = {
    finalState: "error",
    closeReason: "internal_error",
    message: reason
  }): Promise<void> {
    await this.cleanup(socket, closure);
    try { socket.close(code, reason.slice(0, 123)); } catch { /* already closed */ }
  }

  private async cleanup(socket: WebSocket, closure: SessionClosure): Promise<void> {
    this.clearDeadline(socket);
    this.startingSockets.delete(socket);
    this.authorizations.delete(socket);
    const live = this.sessions.get(socket);
    ...
    await Promise.allSettled([
      ...
      this.releaseSessionLease(live.authorization.profile.ownerId, live.authorization.sessionId),
      live.audit.finish(closure.finalState, closure.closeReason, closure.message)
    ]);
  }
  ...
  private async updateSessionLease(
    ...
    if (!secret) throw new Error("SESSION_HMAC_KEY is required for SSH session coordination");
    const id = this.env.SSH_SESSION_REGISTRY.idFromName(ownerId);
    const response = await this.env.SSH_SESSION_REGISTRY.get(id).fetch(`https://ssh-registry.internal/${operation}`, {
      method: "POST",
      headers: { "content-type": "application/json", "x-internal-auth": secret },
      body: JSON.stringify({ sessionId, ...(maximum === undefined ? {} : { maximum }) })
    });
    if (response.status === 409) return { acquired: false, expiresAt: 0 };
    if (!response.ok) throw new Error(`SSH session registry ${operation} failed with status ${response.status}`);
    const result = await response.json<{ acquired?: unknown; expiresAt?: unknown }>();
    if (result.acquired !== true || typeof result.expiresAt !== "number" || !Number.isFinite(result.expiresAt)) {
      throw new Error(`SSH session registry ${operation} returned an invalid lease`);
    }
    return { acquired: true, expiresAt: result.expiresAt };
  }

  private async releaseSessionLease(ownerId: string, sessionId: string): Promise<void> {
    const secret = this.env.SESSION_HMAC_KEY?.trim();
    if (!secret) return;
    const id = this.env.SSH_SESSION_REGISTRY.idFromName(ownerId);
    const response = await this.env.SSH_SESSION_REGISTRY.get(id).fetch("https://ssh-registry.internal/release", {
      method: "POST",
      headers: { "content-type": "application/json", "x-internal-auth": secret },
      body: JSON.stringify({ sessionId })
    });
    if (!response.ok) throw new Error(`SSH session registry release failed with status ${response.status}`);
  }

  private async recordSecurityEvent(ownerId: string, code: string, message: string): Promise<void> {
    try {
      await createRepositories(this.env).securityEvents.append({ ownerId, code, sourceIpHash: null, message });
    } catch (error) {
  ...
  private async recoverRestartedSocket(socket: WebSocket): Promise<void> {
    try {
      ...
      if (attachment?.phase !== "waiting" && attachment?.sessionId) {
        ...
        ).bind(attachment.sessionId).first<RecoveredSessionRow>();
        if (row) {
          ...
          const finalize = (async () => {
            try {
              ...
              await repositories.connectionSessions.finish(row.owner_id, attachment.sessionId, {
                finalState: "error",
        ...
        if (ownerId) {
          await this.releaseSessionLease(ownerId, attachment.sessionId).catch(() => undefined);
        }
      }
    } catch (error) {
      console.error("SSH restart recovery failed", asError(error).message);
    } finally {
      try { socket.close(1012, "Worker restarted; reconnect required"); } catch { /* already closed */ }
    }
  }
...
class D1SSHProfileRepository implements SSHProfileRepository {
  ...
  async resolve(ownerId: string, profileId: string, ephemeral?: EphemeralSSHCredential): Promise<SSHConnectionProfile> {
    const [row, settings] = await Promise.all([
      ...
      this.env.DB.prepare("SELECT collect_commands FROM settings WHERE owner_id = ? LIMIT 1")
        .bind(ownerId).first<SettingsRow>()
    ]);
    ...
    if (ephemeral?.method === "password") {
      ...
      authentication = {
        kind: "password",
        password: await decryptProfileField(this.env, row, "password", ownerId, profileId)
      };
    } else if (row.auth_kind === "private_key") {
      const privateKey = await decryptProfileField(this.env, row, "privateKey", ownerId, profileId);
      const passphrase = hasEnvelope(row, "passphrase")
        ? await decryptProfileField(this.env, row, "passphrase", ownerId, profileId) : undefined;
...
class D1HostKeyRepository implements HostKeyRepository {
  ...
  async get(reference: HostKeyReference): Promise<HostKeyRecord | null> {
    const row = await this.database.prepare(`
      ...
      LIMIT 1
    `).bind(reference.profileId, reference.ownerId, reference.host, reference.port).first<KnownHostRow>();
    return row ? {
      ...reference,
  ...
  async pinIfAbsent(record: HostKeyRecord): Promise<HostKeyRecord> {
    ...
    `).bind(
      crypto.randomUUID(), record.host, record.port, record.keyType, record.fingerprint,
      record.keyBlob, now, now, record.profileId, record.ownerId
    ).run();
    ...
    if (pinned.fingerprint === record.fingerprint) {
      ...
      `).bind(record.fingerprint, now, record.profileId, record.ownerId).run();
    }
    return pinned;
  }
}
...
function hasEnvelope(row: ProfileRow, field: "password" | "privateKey" | "passphrase"): boolean {
  const prefix = field === "privateKey" ? "private_key" : field;
  return row[`${prefix}_ciphertext` as keyof ProfileRow] !== null
...
async function decryptProfileField(
  ...
  if (typeof ciphertext !== "string" || typeof iv !== "string" || version !== 1) {
    throw new Error(`SSH profile ${field} credential is missing`);
  }
  return decryptSecret(env.CREDENTIAL_MASTER_KEY, { version, iv, ciphertext } as EncryptedEnvelope, {
    ownerId,
    recordId: profileId,
    field
  });
...
function toEphemeralCredential(value: EphemeralCredential | undefined): EphemeralSSHCredential | undefined {
  if (!value) return undefined;
  return value.method === "password"
    ? { method: "password", password: value.password }
    : { method: "private_key", privateKey: value.privateKey, passphrase: value.passphrase };
}

async function sealProfile(keyBytes: Uint8Array, profile: SSHConnectionProfile, additionalData: Uint8Array): Promise<TicketEnvelope> {
...
async function openProfile(keyBytes: Uint8Array, envelope: TicketEnvelope, additionalData: Uint8Array): Promise<SSHConnectionProfile> {
  ...
  try {
    return JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(plaintext)) as SSHConnectionProfile;
  } finally {
    plaintext.fill(0);
  }
}

function ticketAdditionalData(sessionId: string, attemptId: string, expiresAt: number, origin: string): Uint8Array {
  return new TextEncoder().encode(`edgesh-ticket-v1:${sessionId}:${attemptId}:${expiresAt}:${origin}`);
}

async function sha256Base64Url(value: Uint8Array): Promise<string> {
  return encodeBase64Url(new Uint8Array(await crypto.subtle.digest("SHA-256", toArrayBuffer(value))));
}
...
function decodeBinaryFrame(bytes: Uint8Array): { header: BinaryFrameHeader; payload: Uint8Array } {
  if (bytes.byteLength < 5) throw new ProtocolError("VALIDATION_FAILED", "Binary frame is truncated");
  const headerBytes = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength).getUint32(0);
  if (headerBytes < 2 || headerBytes > 16 * 1024 || 4 + headerBytes >= bytes.byteLength) {
    throw new ProtocolError("VALIDATION_FAILED", "Binary frame header is invalid");
  }
  let decoded: unknown;
  try { decoded = JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(bytes.subarray(4, 4 + headerBytes))); }
  catch { throw new ProtocolError("VALIDATION_FAILED", "Binary frame header is invalid"); }
  const result = BinaryFrameHeaderSchema.safeParse(decoded);
  if (!result.success) throw new ProtocolError("VALIDATION_FAILED", "Binary frame header is invalid");
  const payload = bytes.subarray(4 + headerBytes);
  if (payload.byteLength !== result.data.payloadBytes) throw new ProtocolError("VALIDATION_FAILED", "Binary frame payload size mismatch");
  return { header: result.data, payload };
}

function encodeBinaryFrame(header: BinaryFrameHeader, payload: Uint8Array): Uint8Array {
...
function mapEngineState(phase: string): "tcp_connecting" | "ssh_handshake" | "authenticating" | "connected" {
...
function createDirectoryPage<T>(entries: T[], offset: number): T[] {
  const page: T[] = [];
  let encodedBytes = 0;
  for (let index = offset; index < entries.length && page.length < 500; index += 1) {
...
function classifyEngineFailure(code: string, message: string): SessionClosure {
  ...
  if (value.includes("host_key") || value.includes("host key")) {
    return { finalState: "error", closeReason: "host_key_rejected", message: "SSH host key validation failed" };
  }
  if (value.includes("auth") || value.includes("permission denied")) {
    return { finalState: "error", closeReason: "authentication_failed", message: "SSH authentication failed" };
  }
  if (value.includes("keepalive")) {
    return { finalState: "error", closeReason: "keepalive_failed", message: "SSH keepalive failed" };
  }
  if (value.includes("timeout") || value.includes("timed out")) {
    return { finalState: "error", closeReason: "connection_timeout", message: "SSH connection timed out" };
  }
  return { finalState: "error", closeReason: "network_error", message: "SSH connection failed" };
}
