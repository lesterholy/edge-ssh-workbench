...
describe("SSH session lease registry", () => {
  it("enforces a per-owner limit while allowing idempotent reacquisition", () => {
    ...
    expect(acquired).toMatchObject({ acquired: true, active: 1, expiresAt: 1_100 });
    expect(acquireSessionLease(acquired.state, first, 1, 1_010, 100)).toMatchObject({ acquired: true, active: 1 });
    expect(acquireSessionLease(acquired.state, second, 1, 1_010, 100)).toMatchObject({ acquired: false, active: 1 });
  });

  it("renews, releases, and reclaims expired leases", () => {
    const acquired = acquireSessionLease(undefined, first, 2, 1_000, 100);
    expect(renewSessionLease(acquired.state, first, 1_050, 100).expiresAt).toBe(1_150);
    ...
    expect(renewSessionLease(acquired.state, first, 1_101, 100).acquired).toBe(false);
  });
});
