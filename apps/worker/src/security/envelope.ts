...
function encodeContext(context: EnvelopeContext): Uint8Array<ArrayBuffer> {
	if (!context.ownerId || !context.recordId)
		throw new Error("Envelope context is incomplete");
	return toArrayBufferView(
		encoder.encode(
			`edgesh:v${CREDENTIAL_ENVELOPE_VERSION}:${context.ownerId}:${context.recordId}:${context.field}`,
		),
	);
}
...
export async function encryptSecret(
	...
	if (!masterKey)
		throw new Error("CREDENTIAL_MASTER_KEY is required to store credentials");
			...
			await crypto.subtle.encrypt(
				{
					name: "AES-GCM",
					iv,
					additionalData: encodeContext(context),
					tagLength: 128,
				},
				key,
				plaintextBytes,
...
export async function decryptSecret(
	...
	if (!masterKey)
		throw new Error("CREDENTIAL_MASTER_KEY is required to decrypt credentials");
