...
function parseProfileId(value: string): string {
	...
	if (!result.success)
		throw new HttpError(400, "VALIDATION_FAILED", "Invalid profile identifier");
	return result.data;
}

function needsMasterKeyForUpdate(
	credential: ProfileUpdateRequest["credential"],
): boolean {
...
function assertTailscaleSshProfile(
	env: Env,
	method: string,
	port: number,
): void {
	...
	if (port !== 22) {
		throw new HttpError(
			400,
			"VALIDATION_FAILED",
			"Tailscale SSH profiles must use port 22",
		);
	}
}
...
async function listProfiles(
	...
	try {
		...
		const page = await new ProfileRepository(
			env.DB,
...
async function updateProfile(
	...
	if (
		input.credential?.method !== "tailscale_ssh" &&
		input.credential?.persistence === "saved"
	) {
		const secret =
			input.credential.method === "password"
		...
		if (
			secret.action === "clear" ||
			(secret.action === "keep" && !alreadyStored)
		) {
			throw new HttpError(
				400,
				"PROFILE_CREDENTIAL_REQUIRED",
				"A saved credential is required for this authentication method",
			);
		}
	}
...
async function deleteProfile(
	...
	if (!(await repository.deleteIfNoActiveSessions(ownerId, profileId))) {
		throw new HttpError(
			409,
			"CONFLICT",
			"Disconnect active SSH sessions before deleting this profile",
		);
	}
...
export async function routeProfiles(
	...
	if (path === "/api/profiles") {
		if (request.method === "GET")
			return listProfiles(request, env, auth.ownerId);
		if (request.method === "POST")
			return createProfile(request, env, auth.ownerId);
	...
	if (request.method === "PATCH")
		return updateProfile(request, env, auth.ownerId, profileId);
	if (request.method === "DELETE")
		return deleteProfile(env, auth.ownerId, profileId);
