...
function importCredential(method: ProfileCreateRequest["credential"]["method"]): ProfileCreateRequest["credential"] {
  if (method === "tailscale_ssh") return { method: "tailscale_ssh" };
  if (method === "password") return { method: "password", persistence: "prompt" };
  return { method: "private_key", persistence: "prompt", savePassphrase: false };
}

async function listDevices(env: Env): Promise<Response> {
  return apiJson(TailscaleDeviceListResponseSchema.parse(await fetchTailscaleDevices(env)));
}

async function importDevices(request: Request, env: Env, ownerId: string): Promise<Response> {
  const input = await parseJson(request, TailscaleImportRequestSchema);
  if (env.SSH_TRANSPORT?.trim() !== "tailnet_connector") {
    throw new HttpError(400, "VALIDATION_FAILED", "Tailscale imports require tailnet_connector transport");
  }
  const runtime = getRuntimeConfig(env);
  assertAllowedSshPort(input.port, runtime.allowedSshPorts);

  const discovery = await fetchTailscaleDevices(env);
  const available = new Map(discovery.devices.map((device) => [device.id, device]));
  ...
  const existingTargets = new Set(existing.map((profile) =>
  ...
  for (const deviceId of input.deviceIds) {
    ...
    if (!device.authorized) {
      result.skipped.push({ deviceId, name: device.name, reason: "unauthorized" });
      continue;
    }
...
export async function routeTailscale(request: Request, env: Env, path: string): Promise<Response> {
  const auth = await requireAuthentication(request, env);
  if (path === "/api/tailscale/devices") {
    if (request.method === "GET") return listDevices(env);
    return methodNotAllowed(["GET"]);
  }
  if (path === "/api/tailscale/import") {
    if (request.method === "POST") return importDevices(request, env, auth.ownerId);
