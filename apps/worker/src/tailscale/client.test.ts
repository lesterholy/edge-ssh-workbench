...
describe("Tailscale device discovery", () => {
  it("keeps full MagicDNS names and derives a bounded online status", () => {
    const devices = parseTailscaleDevices({ devices: [
      {
        id: "device-1",
        hostname: "alpha",
  ...
  it("does not treat a future last-seen timestamp as online", () => {
    const [device] = parseTailscaleDevices({ devices: [{
      id: "device-future",
      hostname: "future",
  ...
  it("uses HTTP Basic without exposing the token in the URL", async () => {
    const fetcher = vi.fn<typeof fetch>(async () => Response.json({ devices: [] }));
    ...
    const [url, init] = call!;
    expect(url).toBe("https://api.tailscale.com/api/v2/tailnet/example.com/devices");
  ...
  it("reports missing configuration without making a request", async () => {
    const fetcher = vi.fn();
    await expect(fetchTailscaleDevices({}, fetcher as typeof fetch)).rejects.toMatchObject({
      status: 503,
  ...
  it("rejects non-ASCII tokens before constructing the authorization header", async () => {
    const fetcher = vi.fn();
    await expect(fetchTailscaleDevices({
      TAILSCALE_TAILNET: "example.com",
  ...
  it("rejects an oversized response even without a content-length header", async () => {
    const fetcher = vi.fn<typeof fetch>(async () => new Response("x".repeat(2 * 1024 * 1024 + 1)));
