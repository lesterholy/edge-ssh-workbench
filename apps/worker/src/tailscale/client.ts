...
function configuredValue(value: string | undefined, name: string, maxLength: number): string {
  const normalized = value?.trim();
  if (!normalized || normalized.length > maxLength || /[\u0000-\u001f\u007f]/.test(normalized)) {
    throw new HttpError(503, "AUTH_CONFIGURATION_MISSING", `${name} is not configured`);
  }
  return normalized;
}

function configuration(env: Pick<Env, "TAILSCALE_TAILNET" | "TAILSCALE_API_TOKEN">): TailscaleApiConfig {
  const config = {
    tailnet: configuredValue(env.TAILSCALE_TAILNET, "TAILSCALE_TAILNET", 256),
    token: configuredValue(env.TAILSCALE_API_TOKEN, "TAILSCALE_API_TOKEN", 4_096),
  };
  if (!/^[\x21-\x7e]+$/.test(config.token)) {
    throw new HttpError(503, "AUTH_CONFIGURATION_MISSING", "TAILSCALE_API_TOKEN is invalid");
  }
  if (config.tailnet !== "-"
    && !/^[A-Za-z0-9](?:[A-Za-z0-9.@_+-]{0,254}[A-Za-z0-9])?$/.test(config.tailnet)) {
    throw new HttpError(503, "AUTH_CONFIGURATION_MISSING", "TAILSCALE_TAILNET is invalid");
  }
  return config;
}
...
function addresses(value: unknown): string[] {
  ...
  return [...new Set(value.flatMap((entry) => {
    ...
    return address && /^[0-9a-f:.]+$/i.test(address) ? [address.toLowerCase()] : [];
  }))].slice(0, 16);
}

export function parseTailscaleDevices(payload: unknown, now = Date.now()): TailscaleDevice[] {
  const root = record(payload);
  if (!root || !Array.isArray(root.devices) || root.devices.length > 5_000) {
    throw new HttpError(502, "SERVICE_UNAVAILABLE", "Tailscale returned an invalid device response", { retryable: true });
  }
  ...
  for (const value of root.devices) {
    ...
    const recentlySeen = Number.isFinite(lastSeenAt) && now >= lastSeenAt && now - lastSeenAt <= ONLINE_WINDOW_MS;
    const online = typeof raw.online === "boolean"
      ? raw.online
...
async function readBoundedBody(response: Response): Promise<string> {
  ...
  try {
    while (true) {
      ...
      if (total > MAX_RESPONSE_BYTES) {
        ...
        throw new HttpError(502, "SERVICE_UNAVAILABLE", "Tailscale device response is too large", { retryable: true });
      }
      chunks.push(value);
    }
  } finally {
  ...
  try {
    ...
    throw new HttpError(502, "SERVICE_UNAVAILABLE", "Tailscale returned invalid UTF-8", { retryable: true });
  }
}

export async function fetchTailscaleDevices(
  env: Pick<Env, "TAILSCALE_TAILNET" | "TAILSCALE_API_TOKEN">,
  ...
  try {
    ...
    throw new HttpError(502, "SERVICE_UNAVAILABLE", "Unable to reach the Tailscale API", { retryable: true });
  }

  if (response.status === 401 || response.status === 403) {
    throw new HttpError(502, "AUTH_CONFIGURATION_MISSING", "Tailscale rejected the configured API token");
  }
  if (response.status === 429) {
    throw new HttpError(503, "RATE_LIMITED", "Tailscale API rate limit reached", {
      retryable: true,
      retryAfterSeconds: 30,
    });
  }
  if (!response.ok) {
    throw new HttpError(502, "SERVICE_UNAVAILABLE", "Tailscale device discovery failed", { retryable: true });
  }
  ...
  try {
    ...
    throw new HttpError(502, "SERVICE_UNAVAILABLE", "Unable to read the Tailscale device response", { retryable: true });
  }
  let payload: unknown;
  try {
    payload = JSON.parse(body);
  } catch {
    throw new HttpError(502, "SERVICE_UNAVAILABLE", "Tailscale returned invalid JSON", { retryable: true });
  }
