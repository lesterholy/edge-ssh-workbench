...
function toView(row: ProfileRow): ProfileView {
	...
	const view = {
		...
		hasPrivateKey: row.private_key_ciphertext !== null,
		hasPassphrase: row.passphrase_ciphertext !== null,
		lastConnectedAt: row.last_connected_at,
		lastSuccessfulUsername: row.last_connected_username,
		lastHostKeyFingerprint: row.last_host_fingerprint,
	...
	Object.defineProperties(view, {
		...
		collectHistory: {
			value: asBoolean(row.collect_history),
			enumerable: false,
		},
		lastConnectedUsername: {
			value: row.last_connected_username,
			enumerable: false,
		},
		lastHostFingerprint: {
			value: row.last_host_fingerprint,
			enumerable: false,
		},
	});
...
function fieldEnvelope(
	row: ProfileRow,
	field: Exclude<SecretField, "totp" | "pendingTotp">,
...
async function resolveSecret(
	...
	return encryptSecret(masterKey, nextValue, {
		ownerId,
		recordId: profileId,
		field,
	});
...
export class ProfileRepository {
	...
	async list(ownerId: string, limit = 200): Promise<ProfileView[]> {
		...
		const result = await this.db
			.prepare(
				"SELECT * FROM profiles WHERE owner_id = ? ORDER BY updated_at DESC, id DESC LIMIT ?",
			)
			.bind(ownerId, bounded)
			.all<ProfileRow>();
	...
	async listTargetsByHosts(
		...
		if (normalizedHosts.length > 50)
			throw new Error("Too many profile target hosts");
	...
	async create(
		...
		const authKind = validateEnum(
			input.authKind,
			["password", "private_key", "tailscale_ssh"],
			"auth kind",
		);
		if (authKind === "tailscale_ssh" && port !== 22)
			throw new Error("Tailscale SSH profiles must use port 22");
		if (
			authKind === "tailscale_ssh" &&
			(input.password || input.privateKey || input.passphrase)
		) {
			throw new Error("Tailscale SSH profiles must not contain credentials");
		}
		const storedAuthKind: StoredProfileAuthKind =
			authKind === "tailscale_ssh" ? "password" : authKind;
		const storedPersistence: StoredCredentialPersistence =
			authKind === "tailscale_ssh"
				? "saved"
				: validateEnum(
						input.credentialPersistence ?? "saved",
						["saved", "prompt"],
						"credential persistence",
					);
		const now = nowIso();
		const password = input.password
			? await encryptSecret(this.masterKey, input.password, {
					ownerId,
					recordId: id,
					field: "password",
				})
		...
		const privateKey = input.privateKey
			? await encryptSecret(this.masterKey, input.privateKey, {
					ownerId,
					recordId: id,
					field: "privateKey",
				})
		...
		const passphrase = input.passphrase
			? await encryptSecret(this.masterKey, input.passphrase, {
					ownerId,
					recordId: id,
					field: "passphrase",
				})
		...
		const row: ProfileRow = {
			id,
			owner_id: ownerId,
			name: validateText(input.name, "name", 100),
			host: normalizeHost(input.host),
			port,
			username: validateText(input.username, "username", 128),
			auth_kind: storedAuthKind,
			...
			initial_command:
				input.initialCommand == null
					? null
					: validateText(input.initialCommand, "initial command", 8192, true),
			terminal_type: validateEnum(
				input.terminalType ?? "xterm-256color",
				["xterm-256color", "xterm", "screen-256color"],
				"terminal type",
			),
			encoding: validateEnum(
				input.encoding ?? "utf-8",
				["utf-8", "gb18030", "big5"],
				"encoding",
			),
			collect_history: toInteger(input.collectHistory ?? true),
			password_ciphertext: password?.ciphertext ?? null,
			password_iv: password?.iv ?? null,
			password_version: password?.version ?? null,
			private_key_ciphertext: privateKey?.ciphertext ?? null,
			private_key_iv: privateKey?.iv ?? null,
			private_key_version: privateKey?.version ?? null,
			passphrase_ciphertext: passphrase?.ciphertext ?? null,
			passphrase_iv: passphrase?.iv ?? null,
			passphrase_version: passphrase?.version ?? null,
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NULL, NULL, NULL, ?, ?)`,
			...
			.bind(
				...
				row.private_key_iv,
				row.private_key_version,
				row.passphrase_ciphertext,
				row.passphrase_iv,
				row.passphrase_version,
				row.created_at,
				row.updated_at,
			)
			.run();
	...
	async createFromRequest(
		...
		const shouldSave =
			request.credential.method !== "tailscale_ssh" &&
			request.credential.persistence === "saved";
		return this.create(ownerId, {
			...
			credentialPersistence:
				request.credential.method === "tailscale_ssh"
					? "none"
					: request.credential.persistence,
			notes: request.notes,
			initialCommand: request.initialCommand,
			terminalType: request.terminalType,
			encoding: request.encoding,
			password:
				request.credential.method === "password" && shouldSave
					? request.credential.password
					: undefined,
			privateKey:
				request.credential.method === "private_key" && shouldSave
					? request.credential.privateKey
					: undefined,
			passphrase:
				request.credential.method === "private_key" &&
				shouldSave &&
				request.credential.savePassphrase
	...
	async update(
		...
		if (
			...
			(typeof patch.password === "string" ||
				typeof patch.privateKey === "string" ||
				typeof patch.passphrase === "string")
		) {
			throw new Error("Tailscale SSH profiles must not contain credentials");
		}
		const password =
		...
		const updated: ProfileRow = {
			...
			name:
				patch.name === undefined
					? row.name
					: validateText(patch.name, "name", 100),
			host: patch.host === undefined ? row.host : normalizeHost(patch.host),
			port,
			username:
				patch.username === undefined
					? row.username
					: validateText(patch.username, "username", 128),
			...
			notes:
				patch.notes === undefined
					? row.notes
					: validateText(patch.notes, "notes", 4000, true),
			initial_command:
				patch.initialCommand === undefined
					? row.initial_command
					: patch.initialCommand === null
						? null
						: validateText(patch.initialCommand, "initial command", 8192, true),
			terminal_type:
				patch.terminalType === undefined
					? row.terminal_type
					: validateEnum(
							patch.terminalType,
							["xterm-256color", "xterm", "screen-256color"],
			...
			collect_history:
				patch.collectHistory === undefined
					? row.collect_history
					: toInteger(patch.collectHistory),
			password_ciphertext: password?.ciphertext ?? null,
			password_iv: password?.iv ?? null,
			password_version: password?.version ?? null,
			private_key_ciphertext: privateKey?.ciphertext ?? null,
        ...
        passphrase_version = ?, updated_at = ? WHERE id = ? AND owner_id = ?`,
			...
			.bind(
				...
				updated.username,
				updated.auth_kind,
				updated.tailscale_ssh,
				updated.credential_persistence,
				updated.notes,
				updated.initial_command,
				updated.terminal_type,
				updated.encoding,
				updated.collect_history,
				updated.password_ciphertext,
				updated.password_iv,
				updated.password_version,
				updated.private_key_ciphertext,
				updated.private_key_iv,
				updated.private_key_version,
				updated.passphrase_ciphertext,
				updated.passphrase_iv,
				updated.passphrase_version,
				updated.updated_at,
				profileId,
				ownerId,
			)
	...
	async deleteIfNoActiveSessions(
		ownerId: string,
		profileId: string,
	): Promise<boolean> {
		const result = await this.db
       ...
       WHERE id = ? AND owner_id = ?
         AND NOT EXISTS (
           SELECT 1 FROM connection_sessions
           WHERE profile_id = ? AND owner_id = ? AND status NOT IN ('closed', 'error')
         )`,
			)
			.bind(profileId, ownerId, profileId, ownerId)
			.run();
	...
	async getCredentials(
		...
		for (const field of ["password", "privateKey", "passphrase"] as const) {
			...
			if (encrypted)
				output[field] = await decryptSecret(this.masterKey, encrypted, {
					ownerId,
					recordId: profileId,
					field,
				});
	...
	async markConnected(
       ...
       WHERE id = ? AND owner_id = ?`,
			...
			.bind(nowIso(), username, fingerprint, nowIso(), profileId, ownerId)
			.run();
	}
	...
	private async getRow(
		...
		return this.db
			.prepare("SELECT * FROM profiles WHERE id = ? AND owner_id = ?")
