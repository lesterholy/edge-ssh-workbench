...
function toCommand(row: CommandRow): CommandHistoryItem {
  return {
    id: row.id, sessionId: row.session_id, profileId: row.profile_id, profileName: row.profile_name_snapshot,
    host: row.host_snapshot, username: row.username_snapshot, command: row.command_redacted,
    captureQuality: row.capture_kind, executedAt: row.created_at,
  };
}

const SECRET_ASSIGNMENT = /\b(password|passwd|passphrase|token|secret|api[_-]?key|access[_-]?key|authorization)\s*(=|:)\s*(?:"[^"]*"|'[^']*'|\S+)/gi;
const SECRET_FLAG = /(--(?:password|passphrase|token|secret|api-key|access-key)(?:=|\s+))(?:"[^"]*"|'[^']*'|\S+)/gi;

export function redactCommand(command: string): string {
  const trimmed = command.trim().slice(0, 8192);
  if (!trimmed) throw new Error("Command is empty");
  if (/-----BEGIN [A-Z ]*PRIVATE KEY-----/i.test(trimmed) || /\b(?:sshpass|set-password)\b/i.test(trimmed)
...
export class CommandHistoryRepository {
  ...
  async append(ownerId: string, input: AppendCommandInput): Promise<CommandHistoryItem> {
    ...
    ).bind(id, ownerId, input.sessionId, input.profileId, input.profileName, input.host, input.username,
      command, input.captureQuality, executedAt, input.sessionId, ownerId).run();
    if (result.meta.changes !== 1) throw new Error("Connection session not found");
    return {
      id, sessionId: input.sessionId, profileId: input.profileId, profileName: input.profileName,
      host: input.host, username: input.username, command, captureQuality: input.captureQuality, executedAt,
    };
  }

  async list(ownerId: string, query: CommandHistoryQuery = {}): Promise<PageResult<CommandHistoryItem>> {
    ...
    if (query.profileId) { clauses.push("profile_id = ?"); bindings.push(query.profileId); }
    if (query.query) { clauses.push("command_redacted LIKE ? ESCAPE '\\'"); bindings.push(`%${escapeLike(query.query)}%`); }
    if (query.from) { clauses.push("created_at >= ?"); bindings.push(query.from); }
    if (query.to) { clauses.push("created_at <= ?"); bindings.push(query.to); }
    if (cursor) {
    ...
    ).bind(...bindings, limit + 1).all<CommandRow>();
    const hasMore = result.results.length > limit;
    const rows = result.results.slice(0, limit);
    const last = rows[rows.length - 1];
    return {
      items: rows.map(toCommand), hasMore,
      nextCursor: hasMore && last ? encodeTimeCursor({ createdAt: last.created_at, id: last.id }) : null,
    };
  }

  async clear(ownerId: string, options: { before?: string; profileId?: string } = {}): Promise<number> {
    const clauses = ["owner_id = ?"];
    const bindings: unknown[] = [ownerId];
    if (options.before) { clauses.push("created_at < ?"); bindings.push(options.before); }
    if (options.profileId) { clauses.push("profile_id = ?"); bindings.push(options.profileId); }
    const result = await this.db.prepare(`DELETE FROM command_history WHERE ${clauses.join(" AND ")}`).bind(...bindings).run();
  ...
  async prune(ownerId: string, batchSize = 500): Promise<number> {
    ...
    ).bind(ownerId, bounded, MAX_COMMAND_HISTORY).run();
    return result.meta.changes;
  }
}
...
export class ConnectionSessionRepository {
  ...
  async start(ownerId: string, input: StartConnectionSessionInput): Promise<SessionHistoryItem> {
    ...
    return {
      ...
      connectedAt: null, endedAt: null, finalState: "authorizing", closeReason: null,
    };
  }

  async setState(ownerId: string, sessionId: string, state: SessionState, connectedAt?: string): Promise<void> {
    await this.db.prepare(
      "UPDATE connection_sessions SET status = ?, connected_at = COALESCE(connected_at, ?) WHERE id = ? AND owner_id = ?",
    ).bind(state, connectedAt ?? null, sessionId, ownerId).run();
  }

  async finish(ownerId: string, sessionId: string, input: FinishConnectionSessionInput): Promise<void> {
    ...
    ).bind(input.finalState, input.closeReason ?? null, input.connectedAt ?? null, endedAt, endedAt,
      input.hostKeyType ?? null, input.hostFingerprint ?? null, input.kexAlgorithm ?? null,
      input.cipherIn ?? null, input.cipherOut ?? null, sessionId, ownerId).run();
  }

  async list(ownerId: string, query: { cursor?: string; limit?: number; profileId?: string; state?: SessionState } = {}): Promise<PageResult<SessionHistoryItem>> {
    const limit = Math.min(Math.max(Math.trunc(query.limit ?? 50), 1), 100);
    const cursor = decodeTimeCursor(query.cursor);
    const clauses = ["owner_id = ?"];
    const bindings: unknown[] = [ownerId];
    if (query.profileId) { clauses.push("profile_id = ?"); bindings.push(query.profileId); }
    if (query.state) { clauses.push("status = ?"); bindings.push(query.state); }
    if (cursor) {
      clauses.push("(created_at < ? OR (created_at = ? AND id < ?))");
      bindings.push(cursor.createdAt, cursor.createdAt, cursor.id);
    }
    ...
    ).bind(...bindings, limit + 1).all<SessionRow>();
    const hasMore = result.results.length > limit;
    const rows = result.results.slice(0, limit);
    const last = rows[rows.length - 1];
    return { items: rows.map(toSession), hasMore, nextCursor: hasMore && last ? encodeTimeCursor({ createdAt: last.started_at, id: last.id }) : null };
  }
}
...
function safeEventMessage(message: string): string {
  ...
  return value.replace(SECRET_ASSIGNMENT, (_match, name: string, separator: string) => `${name}${separator}[REDACTED]`);
}

export class SessionEventRepository {
  constructor(private readonly db: D1Database) {}

  async append(ownerId: string, sessionId: string, code: SessionEventCode, message: string): Promise<SessionEvent> {
    const event: SessionEvent = { id: createId("evt"), sessionId, code, message: safeEventMessage(message), createdAt: nowIso() };
    const result = await this.db.prepare(
    ...
    ).bind(event.id, ownerId, sessionId, code, event.message, event.createdAt, sessionId, ownerId).run();
    if (result.meta.changes !== 1) throw new Error("Connection session not found");
    return event;
  }

  async list(ownerId: string, sessionId: string, limit = 500): Promise<SessionEvent[]> {
    ...
    ).bind(ownerId, sessionId, bounded).all<EventRow>();
    return result.results.map((row) => ({ id: row.id, sessionId: row.session_id, code: row.event_code, message: row.message_safe, createdAt: row.created_at }));
  }
}
