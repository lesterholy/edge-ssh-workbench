...
describe("tailscaleStatusFromDevices", () => {
	it("summarizes online and offline device counts", () => {
		expect(
			tailscaleStatusFromDevices(
				response([device("a", true), device("b", false), device("c", true)]),
			),
		).toEqual({
			kind: "ready",
...
describe("tailscaleStatusFromError", () => {
	it("maps missing Tailscale configuration to not_configured", () => {
		const error = new ApiError(
			"TAILSCALE_API_TOKEN is not configured",
	...
	it("maps non-API failures to error", () => {
		expect(tailscaleStatusFromError(new TypeError("network down"))).toEqual({
			kind: "error",
		});
