...
export function ProfileSidebar({
	...
	onUpdate,
	onDelete,
	onTailscaleImportOpen,
	tailscaleRefreshSignal,
}: Props) {
	...
	function startEdit(profile: ProfileResponse) {
		...
		setDraft({
			...
			persistence:
				profile.credentialPersistence === "none"
					? "saved"
					: profile.credentialPersistence,
			password: "",
			privateKey: "",
			passphrase: "",
			notes: profile.notes,
	...
	async function submit(event: FormEvent) {
		...
		try {
			if (editingId) {
				const credential: ProfileUpdateRequest["credential"] =
							...
							? {
									...
									password:
										draft.persistence === "prompt"
											? { action: "clear" }
											: draft.password
												? { action: "replace", value: draft.password }
												: { action: "keep" },
								}
							: {
									method: "private_key",
									...
									privateKey:
										draft.persistence === "prompt"
											? { action: "clear" }
											: draft.privateKey
												? { action: "replace", value: draft.privateKey }
												: { action: "keep" },
									passphrase:
										draft.persistence === "prompt"
											? { action: "clear" }
											: draft.passphrase
												? { action: "replace", value: draft.passphrase }
												: { action: "keep" },
								};
				...
				const credential: ProfileCreateRequest["credential"] =
					draft.method === "tailscale_ssh"
						? { method: "tailscale_ssh" }
						: draft.method === "password"
							? draft.persistence === "prompt"
							...
							: {
									...
									passphrase:
										draft.persistence === "saved"
											...
											: undefined,
									savePassphrase:
										draft.persistence === "saved" && Boolean(draft.passphrase),
								};
				await onCreate({
					name: draft.name,
					host: draft.host,
		...
		<aside className="server-sidebar">
			<div className="section-heading">
				...
				<button
					className="icon-button"
					type="button"
					title={t("importFromTailscale")}
					disabled={busy}
					onClick={() => onTailscaleImportOpen?.()}
				>
					<Import size={17} />
				</button>
			</div>
			<TailscaleStatusBadge
				t={t}
				refreshSignal={tailscaleRefreshSignal}
				onOpenImport={onTailscaleImportOpen}
			/>
			...
			<div className="server-list">
				{filtered.map((profile) => (
					<div
						className={`server-row${selectedId === profile.id ? " selected" : ""}`}
						key={profile.id}
					>
						<button
						...
						>
							<span className="server-avatar" aria-hidden="true">
								{(profile.name || profile.host).slice(0, 1).toUpperCase()}
							</span>
							<span className="server-meta">
								<span>
									{profile.name}
									{sessionCounts[profile.id] ? (
										<small className="session-count" title={t("sessions")}>
											{sessionCounts[profile.id]}
										</small>
									) : null}
								</span>
						...
						<div className="row-actions">
							...
							<button
								...
								disabled={busy || activeProfileIds.has(profile.id)}
								onClick={() =>
				...
				{filtered.length === 0 ? (
					<p className="empty-state">{t("noServer")}</p>
				) : null}
			</div>
			{formOpen ? (
				<form className="profile-form" onSubmit={submit}>
					...
					<div className="split-fields">
						<input
							required
							disabled={draft.method === "tailscale_ssh"}
							type="number"
							min={1}
							max={65535}
							value={draft.port}
					...
					{draft.method !== "tailscale_ssh" ? (
						<select
							...
							onChange={(event) =>
								setDraft({
									...draft,
									persistence: event.target.value as Draft["persistence"],
									password: "",
									privateKey: "",
									passphrase: "",
								})
					...
					{draft.method !== "tailscale_ssh" &&
					draft.persistence === "prompt" ? (
						<p className="form-help">{t("promptCredentialHelp")}</p>
					) : null}
					...
					<input
						maxLength={8192}
						placeholder={t("initialCommand")}
						value={draft.initialCommand}
						onChange={(event) =>
							setDraft({ ...draft, initialCommand: event.target.value })
						}
					/>
					<textarea
						rows={2}
